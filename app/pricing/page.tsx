    ...
    <>
      <PageHero
        ...
        title={
          <>
            투명한 투자 구조,
            <br />
            <span className="text-gradient">맞는 입구</span>가 있습니다.
          </>
        }
      ...
      <section className="container-x py-16 md:py-20">
        <div className="grid items-stretch gap-5 lg:grid-cols-3">
          {PRICING_PLANS.map((plan, i) => (
            <Reveal key={plan.name} delay={(i % 3) * 80} className="h-full">
      ...
      <section className="container-x py-12 md:py-16">
        <div className="grid gap-5 md:grid-cols-2">
          <GlassCard>
            <p className="label-eyebrow mb-4">개설비 포함 (예시)</p>
            <ul className="grid grid-cols-2 gap-3">
              {INCLUDED.map((x) => (
                <li key={x} className="flex items-center gap-2 text-sm text-cream/75">
          ...
          <GlassCard>
            <p className="label-eyebrow mb-4">별도 (예시)</p>
            <ul className="grid grid-cols-2 gap-3">
              {SEPARATE.map((x) => (
                <li key={x} className="flex items-center gap-2 text-sm text-cream/75">
      ...
      <section className="container-x py-12 md:py-16">
        <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <GlassCard className="border-l-2 border-l-cyan">
            <p className="label-eyebrow mb-3">로열티 구조</p>
          ...
          <div>
            <SectionHeader eyebrow="ROI를 가르는 변수" title="수익률은 약속이 아니라, 변수의 함수입니다." />
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              {ROI.map(([t, d]) => (
                <div key={t} className="rounded-2xl border border-hairline bg-white/[0.03] p-5">
                  <p className="text-sm font-bold text-cream">{t}</p>
                  <p className="mt-1 text-[13px] leading-relaxed text-cream/55">{d}</p>
                </div>
              ))}
            </div>
          </div>
        </div>
        <p className="mt-8 text-[12px] leading-relaxed text-cream/40">{PRICING_DISCLAIMER}</p>
      </section>

      <CTASection
        title={<>투자 구조가 명확해졌다면,<br /><span className="text-gradient">대화를 시작할 시간</span>입니다.</>}
